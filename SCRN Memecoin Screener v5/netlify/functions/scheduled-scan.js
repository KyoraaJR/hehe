...
const MAX_CANDIDATES_PER_RUN = 15; // keep each run well inside the function time limit
const SEEN_CAP = 2000;
...
exports.handler = async function () {
  if (!process.env.VAPID_PUBLIC_KEY || !process.env.VAPID_PRIVATE_KEY) {
    // Not configured yet — no-op instead of erroring the whole cron run.
  ...
  // 1. Discover newly-profiled Solana tokens (closest free/no-auth signal
  //    DexScreener exposes for "a token that just showed up").
  let profiles = [];
  try {
  ...
  // 2. One batched DexScreener call covers up to 30 addresses.
  let pairsByAddr = {};
  try {
  ...
  // 3. RugCheck + scoring per candidate, push to every subscriber on a pass.
  let alerted = 0;
  const newlySeen = [];
  for (const addr of candidates) {
    ...
    } catch (e) { /* no rug data -> mint/freeze/lp checks fail closed, which is correct here */ }
    ...
    for (const sub of subs) {
      try {
        ...
        // 404/410 = subscription expired or unsubscribed elsewhere; drop it silently.
